    ...
    public String getCode()
	{
		String code = "";
          ...
          String getIdMethod = getByIdCode();
           ...
           String getRowMethod = getRow();
	       
	       code = code+ getRowMethod;
	       
	       code = code + "\n";
	       
           String getRowsMethod = getRows();
	       
	       code = code+ getRowsMethod;
...
*/
    ...
    public String getRow()
    {
       String code= "";
    	...
    	CodeManager manager = new CodeManager();
    	code=code +"public function getRow($"+ entity.toLowerCase()+"id)\n";
    	code=code +" {\n";
    	code=code +"   $connection = parent::connect();\n";
    	code=code +"     $selectSQL = \"SELECT * FROM "+entity.toUpperCase()+"WHERE "+entity+"ID=\""+ ".intval($"+entity.toLowerCase()+"id);\n";
    	code=code +"     $rows = $connection->query($selectSQL);\n";
    	code=code +"     $"+entity.toLowerCase()+"s = array();\n";
    	//code=code +"i=0;\n";
    	code=code +"     foreach($rows as $row)\n";
    	code=code +"      {\n";
    	//code = code+"       $"+entity.toLowerCase()+"s[i]["+entity+"id] = $row[0];\n";
    ...
    public String getRows()
    {
       String code= "";
    	...
    	CodeManager manager = new CodeManager();
    	code=code +"public function getRows()\n";
    	code=code +" {\n";
    	code=code +"   $connection = parent::connect();\n";
    	...
    	code=code +"     foreach($rows as $row)\n";
    	code=code +"      {\n";
    	code = code+"       $"+entity.toLowerCase()+"s[$i][\""+entity.toLowerCase()+"id\"] = $row[0];\n";
    	int j;
        for(int i=0; i< entityColumns.size();i++)
   	    {
  		  j = i+1;
  	      code = code+"       $"+entity.toLowerCase()+"s[$i][\""+entityColumns.get(i).toLowerCase()+"\"] = $row["+j+"];\n";
	...
	public static void main(String[] args) {
		...
		*/
		...
		methodDeleteTemplate.setParameters(parameterTemplates);	
		   ...
		   MethodTemplate methodRowsTemplate = manager.createMethodTemplate("getRows", "void", null, "public");
			
			//methodRowsTemplate.setParameters(parameterTemplates);	
			
			   methodTemplates.add(methodRowsTemplate);   
